#include <levelset/SpatialDerivative/UpwindFirst/UpwindFirstWENO5b.hpp>
#include "UpwindFirstWENO5b_impl.hpp"
#include <vector>
#include <cstdint>
#include <cmath>
#include <typeinfo>
...
{
	cache = new UpwindFirstWENO5b_Cache;
...
bool UpwindFirstWENO5b_impl::operator==(const UpwindFirstWENO5b_impl& rhs) const {
...
bool UpwindFirstWENO5b_impl::execute_dim0(
...
bool UpwindFirstWENO5b_impl::execute_dim1(
	/*	beacls::FloatVec& dst_deriv_l,
	beacls::FloatVec& dst_deriv_r,
	const FLOAT_TYPE* src,
	const BoundaryCondition *boundaryCondition,
...
bool UpwindFirstWENO5b_impl::execute_dimLET2(
	/*	beacls::FloatVec& dst_deriv_l,
	beacls::FloatVec& dst_deriv_r,
	const FLOAT_TYPE* src,
	const BoundaryCondition *boundaryCondition,
...
bool UpwindFirstWENO5b_impl::execute(
...
UpwindFirstWENO5b::UpwindFirstWENO5b(
	const HJI_Grid *hji_grid,
	const beacls::UVecType type
	) {
	pimpl = new UpwindFirstWENO5b_impl(hji_grid,type);
}
UpwindFirstWENO5b::~UpwindFirstWENO5b() {
	if (pimpl) delete pimpl;
}

bool UpwindFirstWENO5b::execute(
	beacls::UVec& dst_deriv_l,
	beacls::UVec& dst_deriv_r,
	const HJI_Grid *grid,
	const FLOAT_TYPE* src,
...
bool UpwindFirstWENO5b_impl::synchronize(const size_t dim) {
...
bool UpwindFirstWENO5b::synchronize(const size_t dim) {
	if (pimpl) return pimpl->synchronize(dim);
	else return false;
}

bool UpwindFirstWENO5b::operator==(const UpwindFirstWENO5b& rhs) const {
	if (this == &rhs) return true;
	else if (!pimpl) {
...
bool UpwindFirstWENO5b::operator==(const SpatialDerivative& rhs) const {
	if (this == &rhs) return true;
	else if (typeid(*this) != typeid(rhs)) return false;
	else return operator==(dynamic_cast<const UpwindFirstWENO5b&>(rhs));
}


UpwindFirstWENO5b::UpwindFirstWENO5b(const UpwindFirstWENO5b& rhs) :
	pimpl(rhs.pimpl->clone())
{
}

UpwindFirstWENO5b* UpwindFirstWENO5b::clone() const {
	return new UpwindFirstWENO5b(*this);
}
beacls::UVecType UpwindFirstWENO5b::get_type() const {

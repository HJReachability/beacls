...
bool ShapeCylinder_impl::execute(const HJI_Grid *grid, beacls::FloatVec& data) const {
	...
	for (size_t dimension = 0; dimension < num_of_dimensions; ++dimension) {
		if (std::all_of(ignoreDims.cbegin(), ignoreDims.cend(), [dimension](const auto& rhs) { return rhs != dimension; })) {
			const beacls::FloatVec &xs = grid->get_xs(dimension);

			FLOAT_TYPE center_d = modified_center[dimension];
			std::transform(xs.cbegin(), xs.cend(), data.begin(), data.begin(), ([center_d](const auto &xs_i, const auto &data_i) {
				return data_i + std::pow((xs_i - center_d), 2);
			}));
		}
	}
	std::transform(data.begin(), data.end(), data.begin(), ([this](const auto &data_i) {

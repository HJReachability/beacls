...
OdeCFL2_impl::OdeCFL2_impl(const OdeCFL2_impl& rhs) :
	term(rhs.term->clone()),
...
FLOAT_TYPE OdeCFL2_impl::execute(
	beacls::FloatVec& y,
	...
	const bool enable_user_defined_dynamics_on_gpu)
{
	...
	while ((tspan[1] - t) >= (small * HjiFabs<FLOAT_TYPE>(tspan[1]))) {
		...
		for (size_t i = 0; i < ydot.size(); ++i) {
			y[i] = (FLOAT_TYPE)(0.5 * (y[i] + (y1[i] + deltaT * ydot[i])));
		}
...
OdeCFL2::OdeCFL2(const OdeCFL2& rhs) :
	pimpl(rhs.pimpl->clone())
{
}

OdeCFL2* OdeCFL2::clone() const {
	return new OdeCFL2(*this);
}

...
HJIPDE_impl::HJIPDE_impl(
	const std::string &tmp_filename) : tmp_filename(tmp_filename),
									   windowName("")
{
}
HJIPDE_impl::~HJIPDE_impl()
...
bool helperOC::ExecParameters::operator==(const ExecParameters &rhs) const
{
	if (this == &rhs)
		return true;
	else if (line_length_of_chunk != rhs.line_length_of_chunk)
		return false; //!< Line length of each parallel execution chunks  (0 means set automatically)
	else if (num_of_threads != rhs.num_of_threads)
		return false; //!< Number of CPU Threads (0 means use all logical threads of CPU)
	else if (num_of_gpus != rhs.num_of_gpus)
		return false; //!<  Number of GPUs which (0 means use all GPUs)
	else if (calcTTR != rhs.calcTTR)
		return false; //!< calculate TTR during solving
	else if (delayedDerivMinMax != rhs.delayedDerivMinMax)
		return false; //!< Use last step's min/max of derivatives, and skip 2nd pass.
	else if (useCuda != rhs.useCuda)
		return false; //!< Execute type CPU Vector or GPU.
	else if (enable_user_defined_dynamics_on_gpu != rhs.enable_user_defined_dynamics_on_gpu)
		return false; //!< Flag for user defined dynamics function on gpu
	else
		return true;
}
...
bool HJIPDE_impl::TD2TTR(
	...
	const beacls::FloatVec &tau) const
{
	...
	else if (tmp_filename.empty())
	{
		...
		if (TTR.size() != num_of_elements)
			TTR.resize(num_of_elements);
			...
			const helperOC::HJIPDE_extraArgs& extraArgs)
{
	...
	if (quiet)
	{
		std::cout << "Local Q solve running in quiet mode..." << std::endl;
	}
...
{
	if (pimpl)
		return pimpl->solve(stoptau, extraOuts, datas, tau, schemeData, minWith, extraArgs);
	return false;
}
bool HJIPDE::solve(
	beacls::FloatVec &stoptau,
...
{
	if (pimpl)
		return pimpl->solve(stoptau, extraOuts, std::vector<beacls::FloatVec>{data}, tau, schemeData, minWith, extraArgs);
	return false;
}
bool HJIPDE::solve(
...
{
	if (pimpl)
		return pimpl->get_datas(dst_datas, src_tau, schemeData);
	return false;
}
...
bool HJIPDE_impl::get_last_data(
	beacls::FloatVec &dst_data) const
{
	dst_data = last_data;
	return true;
}
...
{
	if (pimpl)
		return pimpl->get_last_data(dst_data);
	return false;
}
bool HJIPDE::TD2TTR(
	beacls::FloatVec &TTR,

...
bool Plane4D::operator==(const Plane4D& rhs) const {
	...
	else if ((aRange.size() != rhs.aRange.size()) || !std::equal(aRange.cbegin(), aRange.cend(), rhs.aRange.cbegin())) return false;	//!< Acceleration control bounds
...
bool Plane4D::save(
	...
	if (!aRange.empty()) result &= save_vector(aRange, std::string("aRange"), beacls::IntegerVec(), true, fs, variable_ptr);
...
bool Plane4D::optCtrl(
	...
	bool result = true;
	result &= optCtrl0_cell_helper(uOpts[0], deriv_ptrs, deriv_sizes, modified_uMode, src_target_dim2_index);
	result &= optCtrl1_cell_helper(uOpts[1], deriv_ptrs, deriv_sizes, modified_uMode, src_target_dim3_index);
	return result;
}
bool Plane4D::optDstb(
	...
	dOpts.resize(get_nd());
	bool result = true;
	result &= optDstb0_cell_helper(dOpts[0], deriv_ptrs, deriv_sizes, modified_dMode, src_target_dim0_index);
	result &= optDstb1_cell_helper(dOpts[1], deriv_ptrs, deriv_sizes, modified_dMode, src_target_dim1_index);
	return result;
}

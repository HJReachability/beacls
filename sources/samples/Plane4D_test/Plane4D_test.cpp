...
int main(int argc, char *argv[])
{
  ...
  helperOC::Plane4D* pl = new helperOC::Plane4D(initState, wMax, aRange, dMax);

  const FLOAT_TYPE inf = std::numeric_limits<FLOAT_TYPE>::infinity();
  // Grid
  beacls::FloatVec 
    gmin{(FLOAT_TYPE)0,(FLOAT_TYPE)0,(FLOAT_TYPE)0,(FLOAT_TYPE)(-1)};
  beacls::FloatVec 
  ...
  levelset::ShapeCylinder(beacls::IntegerVec{2, 3}, 
    beacls::FloatVec{75., 50., 0., 0.}, (FLOAT_TYPE)10).execute(g, targets[0]);

  // Obstacle
  beacls::FloatVec obs1, obs2;
  levelset::ShapeRectangleByCorner(beacls::FloatVec{300, 250, -inf, -inf}, 
    beacls::FloatVec{ 350, 300, inf, inf}).execute(g, obs1);
  levelset::ShapeRectangleByCorner(beacls::FloatVec{5, 5, -inf, -inf}, 
    beacls::FloatVec{ 145, 145, inf, inf}).execute(g, obs2);
  ...
  std::transform(obs1.cbegin(), obs1.cend(), obs2.cbegin(), 
    obstacles[0].begin(), std::ptr_fun<const FLOAT_TYPE&, const FLOAT_TYPE&>(std::min<FLOAT_TYPE>));
  ...
  extraArgs.plotData.projpt = beacls::FloatVec{pl->get_x()[2], pl->get_x()[3]};
  extraArgs.deleteLastPlot = true;
  extraArgs.fig_filename = "figs/Plane4D_test_BRS";
